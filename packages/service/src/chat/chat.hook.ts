...
const useChatWidget = (): ChatWidgetHook => {
  ...
  const sendMessage = async (message: string) => {
    ...
    try {
        ...
        if (functionCalls.length > 0) {
          for (const call of functionCalls) {
            if (call.name === "getCampaignData") {
              const toolCallResult = await getCampaignDataAPI({ ...call.args });
              functionResult = { campaignDetails: toolCallResult };
              functionCall = call;
            }
          }
        }
      ...
      addMessage({ id: new Date().toISOString(), content: text, isBot: true, timestamp: new Date() });
