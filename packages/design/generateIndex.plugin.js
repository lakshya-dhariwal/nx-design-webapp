            ...
            function walk(dir) {
                ...
                files.forEach((file) => {
                    ...
                    if (stat.isDirectory()) {
                        ...
                        const namedExportsMatch =
                            fileContent.match(/export const (\w+)/g);
                        const namedExports = namedExportsMatch
                            ? namedExportsMatch.map(
                                  (match) => match.split(" ")[2]
                              )
                            : [];
                        ...
                        if (hasDefaultExport) {
                            ...
                            console.log(
                                `Added default export for component: ${componentName}`
                            );
                        }

                        if (namedExports.length > 0) {
                            exportStatements.push(
                                `export { ${namedExports.join(", ")} } from './${relativePath}';`
                            );
                            console.log(
                                `Added named exports for component: ${componentName}: ${namedExports.join(", ")}`
                            );
                        }

                        if (!hasDefaultExport && namedExports.length === 0) {
                            console.log(
                                `No exports found in component: ${componentName}`
                            );
                        }
                    }
                });
