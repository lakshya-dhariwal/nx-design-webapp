...
export const BidirectionalSlider: React.FC<BidirectionalSliderProps> = ({
    ...
    const visualToValue = (visualPos: number) => {
        const val =
            visualPos <= 50
                ? min +
                  Math.round((visualPos / 50) * ((origin - min) / minGap)) *
                      minGap
                : origin +
                  Math.round(
                      ((visualPos - 50) / 50) * ((max - origin) / maxGap)
                  ) *
                      maxGap;
        ...
        if (val <= origin) {
            return Number(
                Math.max(min, Math.min(origin, val)).toFixed(decimals)
            );
        } else {
            return Number(
                Math.min(max, Math.max(origin, val)).toFixed(decimals)
            );
        }
    };
        ...
        <div className="">
            <div className="">
                <div className="relative">
                    ...
                    {Array.from({
                        length: Math.floor((origin - min) / minGap) + 1,
                    }).map((_, index) => {
                        ...
                        return (
                            <div
                                key={`left-${index}`}
                                className="absolute top-[18px] h-[5px] w-0.5 -translate-y-1/2 bg-gray-500 opacity-0"
                                style={{
                                    left: `${position}%`,
                    ...
                    {Array.from({
                        length: Math.floor((max - origin) / maxGap) + 1,
                    }).map((_, index) => {
                    ...
                    {
                        <div
                            className="absolute -bottom-6 rounded px-2 py-1 text-sm font-bold text-white"
                            style={{
            ...
            `}
                    ...
                    <input
                        ...
                        onChange={(e) => {
                            const newValue = visualToValue(
                                Number(e.target.value)
                            );
                            onChange(
                                newValue,
                                Math.round(getBidirectionalValue(newValue))
                            );
                        }}
                        className="h-[3px] w-full cursor-pointer appearance-none rounded-lg focus:outline-none [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:border-[3px] [&::-webkit-slider-thumb]:border-solid [&::-webkit-slider-thumb]:bg-white [&::-webkit-slider-thumb]:transition-transform [&::-webkit-slider-thumb]:hover:scale-110"
